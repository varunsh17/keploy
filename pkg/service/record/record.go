...
func (r *Recorder) Instrument(ctx context.Context) (uint64, error) {
	...
	appID, err := r.instrumentation.Setup(ctx, r.config.Command, models.SetupOptions{Container: r.config.ContainerName, DockerNetwork: r.config.NetworkName, DockerDelay: r.config.BuildDelay, Envs: r.config.Envs})
	if err != nil {
